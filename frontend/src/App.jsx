  ...
  const renderIntelligenceBlock = (rawText, accentColor, badgeLabel) => {
    ...
    return cleanString.split('\n').map((paragraph, idx) => {
      ...
      if (isHeaderLine) {
        ...
        return (
          <div key={idx} style={{...styles.intelligenceCard, borderLeft: `3px solid ${accentColor}`, boxShadow: `0 4px 14px ${accentColor}10`}}>
            <div style={{...styles.intelligenceCardHeader, color: accentColor}}>
              ...
              <span style={{...styles.briefBadge, color: accentColor, borderColor: accentColor, backgroundColor: `${accentColor}12`}}>{badgeLabel}</span>
            </div>
    ...
    <div style={styles.container}>
      <style>{`
        ...
        .scanning-loader::after {
          ...
          background: linear-gradient(to bottom, transparent, rgba(2, 132, 199, 0.1), transparent);
      ...
      <main style={styles.main}>
        ...
        {/* THREE-COLUMN WORKSPACE WITH FLOATING GLASS CARD AESTHETIC */}
        <div style={styles.mainWorkspaceLayout} className="main-workspace-layout">
          ...
          <section style={styles.controlCard}>
            ...
            <div style={styles.manualForm}>
              ...
              <div style={styles.grid2Col} className="grid-2col-responsive">
                <div style={styles.sliderGroup}>
                  <label style={styles.label}>Target Return Hurdle: <strong style={{color: '#a855f7'}}>{targetProfit}% CAGR</strong></label>
          ...
          {/* Column 3: Context Drop Zone */}
          <section style={{...styles.controlCard, borderColor: attachedFile ? 'rgba(2, 132, 199, 0.2)' : 'rgba(255, 255, 255, 0.04)'}}>
            ...
            <div style={styles.separatedUploaderLayout}>
              ...
              <div style={{...styles.pdfDropZone, backgroundColor: attachedFile ? 'rgba(2, 132, 199, 0.01)' : '#090d16', borderColor: attachedFile ? 'rgba(2, 132, 199, 0.3)' : '#1a2336'}}>
                {!attachedFile ? (
                  <label style={styles.pdfLabelUpload}>
                    <input type="file" accept=".pdf" onChange={handleFileDropChange} style={{display: 'none'}} />
                    <div style={{fontSize: '22px', marginBottom: '8px', color: '#1a2336', filter: 'drop-shadow(0 0 4px rgba(2, 132, 199, 0.2))'}}>📁</div>
                  ...
                  <div style={styles.fileSuccessRow}>
                    <span style={{fontSize: '14px', color: '#0284c7', fontWeight: 'bold'}}>DATA_ARMED</span>
              ...
              {attachedFile && (
                <div style={styles.activeUploaderStatusBadge}>
                  <span style={styles.uploaderPulseDot}></span> SYSTEM OPERATING: AUDIT MODE
                </div>
              )}
            </div>
          </section>
        ...
        <div style={styles.splitTelemetryRow}>
          ...
          {data && data.market_macro && !attachedFile && (
            <section style={{...styles.educationalCard, borderLeft: '3px solid #0284c7', background: 'linear-gradient(135deg, #090d16 0%, #0c111d 100%)'}}>
        ...
        {/* --- DYNAMIC OUTPUT WORKSPACE DESK --- */}
        {!data && !loading && (
          <div style={styles.emptyStateContainer}>
            <div style={{fontSize: '11px', color: '#475569', letterSpacing: '1px', fontWeight: 'bold'}}>AWAITING PARAMETER DISPATCH TERMINAL IDLE</div>
          </div>
        )}
        ...
        {data && (
          <div style={{marginTop: '24px'}}>
            ...
            {/* MODE A: PORTFOLIO DATA DESK */}
            {!attachedFile && data.optimized_portfolio && (
              <>
                <div style={styles.kpiGrid}>
                  <div style={styles.kpiCard}>
                    <div style={styles.kpiLabel}>COMMITTED CAPITAL BOUNDS</div>
                    <div style={{...styles.kpiValue, color: '#0284c7', textShadow: '0 0 10px rgba(2,132,199,0.15)'}}>{currencySymbol}{(data.profile?.investment_amount || amount).toLocaleString('en-IN')}</div>
                  </div>
                  <div style={styles.kpiCard}>
                    <div style={styles.kpiLabel}>RISK-ADJUSTED MEASURE (SHARPE)</div>
                    <div style={{...styles.kpiValue, color: data.sharpe_ratio > 1.0 ? '#10b981' : '#f59e0b', textShadow: data.sharpe_ratio > 1.0 ? '0 0 10px rgba(16,185,129,0.15)' : 'none'}}>SR {data.sharpe_ratio || 0.0}</div>
                  </div>
                  <div style={styles.kpiCard}>
                    ...
                    <div style={{ width: '100%', height: 250, marginTop: '16px' }}>
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={data.backtest_trajectory}>
                          ...
                          <Tooltip contentStyle={{ backgroundColor: '#090d16', borderColor: '#1a2336', borderRadius: '6px' }} itemStyle={{ color: '#0284c7', fontSize: '12px' }} labelStyle={{ color: '#6e7681', fontSize: '11px' }} formatter={(value) => [`${currencySymbol}${Number(value).toLocaleString('en-IN')}`, 'Portfolio Value']} />
                          <Line type="monotone" dataKey="valuation" stroke="#0284c7" strokeWidth={2.5} dot={false} activeDot={{ r: 4 }} />
                        </LineChart>
                      </ResponsiveContainer>
                    </div>
                  </section>
                        ...
                        <tbody>
                          {data.optimized_portfolio.map((asset, idx) => (
                            <tr key={idx} style={styles.tr}>
                              ...
                              <td style={styles.td}><strong style={{color: '#10b981', fontFamily: 'monospace', fontSize: '14px'}}>{asset.suggested_shares_to_buy}</strong></td>
                ...
                <div style={styles.adversarialGrid} className="adversarial-grid-responsive">
                  <div style={styles.card}>
                    <h3 style={{...styles.cardTitle, color: '#10b981', borderBottom: '1px solid rgba(16,185,129,0.12)'}}>
                      STRATEGIC CONVICTION ANALYSIS
                    </h3>
                    <div style={styles.reportBlock}>{renderIntelligenceBlock(data.report_bull, '#10b981', 'GROWTH')}</div>
                  </div>
                  <div style={styles.card}>
                    <h3 style={{...styles.cardTitle, color: '#ef4444', borderBottom: '1px solid rgba(239,68,68,0.12)'}}>
            ...
            {/* MODE B: DOCUMENT TRACK SECURE DESK */}
            {attachedFile && (
              <div style={{...styles.card, border: '1px solid rgba(2,132,199,0.2)', background: 'linear-gradient(180deg, #090d16 0%, #06090f 100%)', boxShadow: '0 4px 24px rgba(0,0,0,0.3)'}}>
                <h3 style={{...styles.cardTitle, color: '#0284c7', borderBottom: '1px solid rgba(2,132,199,0.15)', paddingBottom: '12px', marginBottom: '12px'}}>
                ...
                <div style={styles.ragDisplayGridResp} className="adversarial-grid-responsive">
                  <div style={styles.ragExtractSubPane}>
                    <h4 style={{fontSize: '11px', color: '#10b981', margin: '0 0 12px 0', fontWeight: 'bold', textTransform: 'uppercase', borderBottom: '1px solid rgba(16,185,129,0.08)', paddingBottom: '6px', letterSpacing: '0.5px'}}>Ingested Highlight Factors</h4>
                  ...
                  <div style={styles.ragExtractSubPane}>
                    <h4 style={{fontSize: '11px', color: '#ef4444', margin: '0 0 12px 0', fontWeight: 'bold', textTransform: 'uppercase', borderBottom: '1px solid rgba(239,68,68,0.08)', paddingBottom: '6px', letterSpacing: '0.5px'}}>Ingested Disclosed Headwinds</h4>
                    <div style={styles.reportBlock}>
...
const styles = {
  container: { backgroundColor: '#06090f', color: '#c9d1d9', minHeight: '100vh', fontFamily: 'system-ui, -apple-system, sans-serif', padding: '24px', boxSizing: 'border-box' },
  header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(255, 255, 255, 0.04)', paddingBottom: '16px', marginBottom: '24px' },
  title: { color: '#e6edf3', fontSize: '18px', fontWeight: '800', letterSpacing: '0.5px', margin: 0 },
  underlineContainer: { display: 'none' },
  centerBlueLine: {},
  subtitle: { color: '#6e7681', fontSize: '12px', marginTop: '4px' },
  statusBadge: { border: '1px solid rgba(255,255,255,0.06)', borderRadius: '4px', padding: '5px 12px', color: '#8b949e', fontSize: '10px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '6px', letterSpacing: '0.5px', textTransform: 'uppercase', backgroundColor: '#090d16' },
  statusDot: { width: '6px', height: '6px', backgroundColor: '#10b981', borderRadius: '50%', display: 'inline-block', boxShadow: '0 0 6px #10b981' },
  main: { maxWidth: '1600px', margin: '0 auto' },
  marketToggleRow: { display: 'flex', gap: '8px', marginBottom: '20px', backgroundColor: '#090d16', padding: '4px', border: '1px solid rgba(255, 255, 255, 0.03)', borderRadius: '6px' },
  ...
  activeMarketTab: { backgroundColor: '#141b2e', color: '#0284c7', border: '1px solid rgba(2, 132, 199, 0.15)', borderRadius: '4px' },
  
  // THREE COLUMN CONTROLLERS WITH REFINED CORNER RADIUS GLUE PLATES
  mainWorkspaceLayout: { display: 'grid', gridTemplateColumns: '1fr 1.1fr 0.9fr', gap: '20px', marginBottom: '20px' },
  controlCard: { backgroundColor: '#090d16', border: '1px solid rgba(255, 255, 255, 0.04)', borderRadius: '8px', padding: '20px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' },
  sectionHeader: { fontSize: '11px', color: '#64748b', textTransform: 'uppercase', marginBottom: '14px', letterSpacing: '0.5px', borderBottom: '1px solid rgba(255, 255, 255, 0.03)', paddingBottom: '6px', fontWeight: '700' },
  ...
  textarea: { backgroundColor: '#06090f', border: '1px solid #1a2336', borderRadius: '6px', color: '#e6edf3', padding: '12px', fontSize: '12px', minHeight: '110px', resize: 'none', outline: 'none', lineHeight: '1.5', fontFamily: 'inherit', transition: 'border-color 0.2s', ':focus': { borderColor: '#0284c7' } },
  
  // FILE INGESTION PLATFORM
  separatedUploaderLayout: { display: 'flex', flexDirection: 'column', height: '100%', justifyContent: 'flex-start' },
  pdfDropZone: { border: '1px dashed #1a2336', borderRadius: '6px', padding: '18px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', transition: 'all 0.2s ease' },
  pdfLabelUpload: { fontSize: '11px', color: '#8b949e', cursor: 'pointer', display: 'block', lineHeight: '1.5' },
  fileSuccessRow: { display: 'flex', alignItems: 'center', backgroundColor: '#06090f', border: '1px solid rgba(2,132,199,0.2)', borderRadius: '6px', padding: '8px 12px', gap: '10px', width: '100%', boxSizing: 'border-box' },
  ...
  activeUploaderStatusBadge: { marginTop: '12px', alignSelf: 'center', backgroundColor: 'rgba(2,132,199,0.03)', border: '1px solid rgba(2,132,199,0.15)', color: '#0284c7', borderRadius: '4px', padding: '4px 10px', fontSize: '9px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '4px', letterSpacing: '0.5px', textTransform: 'uppercase', boxShadow: '0 0 8px rgba(2,132,199,0.05)' },
  uploaderPulseDot: { width: '5px', height: '5px', backgroundColor: '#0284c7', borderRadius: '50%', display: 'inline-block', boxShadow: '0 0 4px #0284c7' },
  
  // PROGRESS LOAD BAR Telemetry
  loadingContainerTrack: { backgroundColor: '#06090f', border: '1px solid rgba(255,255,255,0.03)', borderRadius: '6px', padding: '14px', marginBottom: '20px' },
  loadingBarProgressFilled: { height: '3px', width: '100%', background: 'linear-gradient(90deg, #0284c7, #a855f7, #0284c7)', borderRadius: '2px' },

  // PROFESSIONAL ACTION DISPATCHERS
  submitButtonGreen: { backgroundColor: '#10b981', color: '#ffffff', border: 'none', borderRadius: '6px', padding: '12px 16px', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer', width: '100%', fontFamily: 'inherit', textTransform: 'uppercase', letterSpacing: '0.5px', transition: 'all 0.2s', boxShadow: '0 2px 8px rgba(16,185,129,0.12)', ':hover': { backgroundColor: '#059669', transform: 'translateY(-0.5px)' } },
  submitButtonBlue: { backgroundColor: '#0284c7', color: '#ffffff', border: 'none', borderRadius: '6px', padding: '12px 16px', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer', width: '100%', fontFamily: 'inherit', textTransform: 'uppercase', letterSpacing: '0.5px', transition: 'all 0.2s', boxShadow: '0 2px 8px rgba(2,132,199,0.12)', ':hover': { backgroundColor: '#0369a1', transform: 'translateY(-0.5px)' } },
  submitButtonRed: { backgroundColor: '#dc2626', color: '#ffffff', border: 'none', borderRadius: '6px', padding: '12px 16px', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer', width: '100%', fontFamily: 'inherit', textTransform: 'uppercase', letterSpacing: '0.5px', transition: 'all 0.2s', boxShadow: '0 2px 8px rgba(220,38,38,0.12)', ':hover': { backgroundColor: '#b91c1c' } },
  
  sliderGroup: { display: 'flex', flexDirection: 'column', gap: '4px' },
  label: { fontSize: '11px', color: '#8b949e' },
  select: { backgroundColor: '#06090f', border: '1px solid #1a2336', borderRadius: '6px', color: '#e6edf3', padding: '8px', fontSize: '11px', outline: 'none', fontFamily: 'inherit', cursor: 'pointer' },
  checkboxContainer: { display: 'flex', flexWrap: 'wrap', gap: '6px', backgroundColor: '#06090f', padding: '8px', border: '1px solid #1a2336', borderRadius: '6px' },
  checkboxLabel: { fontSize: '11px', display: 'flex', alignItems: 'center', gap: '4px', cursor: 'pointer', padding: '2px 6px', borderRadius: '4px', backgroundColor: 'rgba(255,255,255,0.01)', border: '1px solid rgba(255,255,255,0.01)' },
  ...
  educationalCard: { backgroundColor: '#090d16', border: '1px solid rgba(255,255,255,0.03)', borderRadius: '8px', padding: '16px', flex: 1, boxShadow: '0 2px 10px rgba(0,0,0,0.05)' },
  ...
  errorCard: { border: '1px solid #ef4444', borderRadius: '8px', color: '#ef4444', padding: '12px', marginBottom: '20px', fontSize: '12px', backgroundColor: 'rgba(239,68,68,0.02)' },
  
  emptyStateContainer: { display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '180px', backgroundColor: '#090d16', border: '1px dashed rgba(255,255,255,0.03)', borderRadius: '8px' },
  kpiGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '14px', marginBottom: '20px' },
  kpiCard: { backgroundColor: '#090d16', border: '1px solid rgba(255, 255, 255, 0.03)', borderRadius: '8px', padding: '14px', display: 'flex', flexDirection: 'column', gap: '4px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' },
  kpiLabel: { color: '#64748b', fontSize: '10px', fontWeight: 'bold', letterSpacing: '0.5px', textTransform: 'uppercase' },
  kpiValue: { fontSize: '18px', fontWeight: '800', fontFamily: 'monospace' },
  card: { backgroundColor: '#090d16', border: '1px solid rgba(255, 255, 255, 0.03)', borderRadius: '8px', padding: '20px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' },
  ...
  tr: { borderBottom: '1px solid rgba(255,255,255,0.01)', transition: 'background-color 0.1s', ':hover': { backgroundColor: 'rgba(255,255,255,0.01)' } },
  td: { padding: '12px 10px', verticalAlign: 'middle' },
  tickerBadge: { backgroundColor: '#06090f', border: '1px solid #1a2336', borderRadius: '4px', padding: '3px 6px', fontWeight: 'bold', color: '#e6edf3', fontFamily: 'monospace' },
  valuationAlertTag: { marginLeft: '8px', border: '1px solid rgba(245,158,11,0.3)', backgroundColor: 'rgba(245,158,11,0.05)', color: '#f59e0b', fontSize: '8px', padding: '1px 5px', borderRadius: '3px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '0.3px' },
  ...
  // TEXT BRIEF REFINED CARDS
  intelligenceCard: { backgroundColor: '#06090f', border: '1px solid rgba(255,255,255,0.02)', borderRadius: '6px', padding: '14px' },
  intelligenceCardHeader: { display: 'flex', alignItems: 'center', gap: '6px', fontSize: '11px', borderBottom: '1px solid rgba(255,255,255,0.03)', paddingBottom: '6px', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.5px', fontWeight: 'bold' },
  intelligenceIndicatorDot: { width: '5px', height: '5px', borderRadius: '50%' },
  briefBadge: { marginLeft: 'auto', fontSize: '8px', border: '1px solid', padding: '1px 5px', borderRadius: '3px', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '0.3px' },
  executionStepRow: { display: 'flex', gap: '8px', alignItems: 'center', backgroundColor: 'rgba(20,27,46,0.12)', padding: '10px', borderRadius: '4px', border: '1px solid rgba(255,255,255,0.01)' },
  narrativeParagraphBlock: { padding: '0 2px' },
  
  ragDisplayGridResp: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' },
  ragExtractSubPane: { backgroundColor: '#06090f', border: '1px solid rgba(255,255,255,0.02)', padding: '16px', borderRadius: '8px' }
};
